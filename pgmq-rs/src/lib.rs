...
//!
//! Bring [`PgMQConnExt`] into scope and call queue methods directly on a connection or
//! transaction:
//!
//! ```ignore
...
//!
//! | Driver | Feature | Async? | Implements `PgMQConnExt` on |
//! |--------|---------|--------|------------------------------|
...
//!
//! All four drivers share the same `PgMQConnExt` trait surface — same method names, same
...
//!
//! ```rust,no_run
//! use pgmq::{PgmqError, Message, PgMQConnExt};
...
//! ### Auto-reborrow
//!
//! `PgMQConnExt` methods take `self` by value, but `Self` is always a reference type
...
//!
//! Every adapter implements `PgMQConnExt` on its driver's transaction type, so you can run
