...
//!
//! Bring [`PGMQueueExt`] into scope and call queue methods directly on a connection or
//! transaction:
//!
//! ```ignore
...
//!
//! | Driver | Feature | Async? | Implements `PGMQueueExt` on |
//! |--------|---------|--------|------------------------------|
...
//!
//! All four drivers share the same `PGMQueueExt` trait surface — same method names, same
...
//! ### Auto-reborrow
//!
//! `PGMQueueExt` methods take `self` by value, but `Self` is always a reference type
...
//!
//! Every adapter implements `PGMQueueExt` on its driver's transaction type, so you can run
