...
mod imp {
    ...
    pub async fn create<C: GenericClient + Sync>(
        ...
        conn.execute(query::CREATE, &[&queue_name]).await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    pub async fn create_unlogged<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        conn.execute(query::CREATE_UNLOGGED, &[&queue_name]).await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    ...
    pub async fn drop_queue<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        conn.execute(query::DROP_QUEUE, &[&queue_name]).await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    pub async fn purge_queue<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        let row = conn.query_one(query::PURGE_QUEUE, &[&queue_name]).await?;
    ...
    >(
        ...
        let rows = conn
            ...
            .await?;
        rows.iter()
            .map(|row| row.try_get("send_batch").map_err(Into::into))
            .collect()
    }
    ...
    >(
        ...
        check_input(queue_name)?;
        let row = conn.query_opt(query::POP, &[&queue_name]).await?;
        row.as_ref()
            .map(Message::<T>::from_tokio_postgres_row)
            .transpose()
    }
    ...
    pub async fn bind_topic<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        conn.execute(query::BIND_TOPIC, &[&pattern, &queue_name])
            .await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    pub async fn unbind_topic<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        conn.execute(query::UNBIND_TOPIC, &[&pattern, &queue_name])
            .await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    ...
    pub async fn enable_notify_insert<C: GenericClient + Sync>(
        ...
        conn.execute(query::ENABLE_NOTIFY_INSERT, &[&queue_name, &ms])
            .await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    pub async fn disable_notify_insert<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        conn.execute(query::DISABLE_NOTIFY_INSERT, &[&queue_name])
            .await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    pub async fn update_notify_insert<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        let ms = duration_as_ms_i32(throttle);
        conn.execute(query::UPDATE_NOTIFY_INSERT, &[&queue_name, &ms])
            .await?;
        Ok(())
    }
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    ...
    pub async fn metrics<C: GenericClient + Sync>(
        ...
        check_input(queue_name)?;
        let row = conn.query_one(query::METRICS, &[&queue_name]).await?;
...
// ---------------------------------------------------------------------------------------------
// Trait impls on `&Client` and `&Transaction<'_>`. Both delegate to the `imp::` shared bodies;
// only the target type differs. Hand-written (no macro) so the signatures are greppable.
// ---------------------------------------------------------------------------------------------

#[async_trait]
impl Queue for &tokio_postgres::Client {
    ...
    async fn convert_archive_partitioned(
        ...
        imp::convert_archive_partitioned(self, table_name, partition_interval, retention_interval)
            .await
    }
    ...
    async fn pop<T: for<'de> Deserialize<'de> + Send + Unpin + 'static>(
        ...
        imp::pop::<_, T>(self, queue_name).await
    }
...
#[async_trait]
impl Queue for &tokio_postgres::Transaction<'_> {
    async fn create(self, queue_name: &str) -> Result<(), PgmqError> {
        imp::create(self, queue_name).await
    }
    async fn create_unlogged(self, queue_name: &str) -> Result<(), PgmqError> {
